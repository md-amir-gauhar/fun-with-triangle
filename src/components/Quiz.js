...
const Quiz = () => {
  ...
  const ans = ['equilateral', 'isosceles', '35sqcm', 'obtuse', 'acute', 'isosceles', 'right', 'perimeter', 'hypotenuse', '5'];
  ...
  return (
    <div className="quiz">
      <Link to="/" className="quiz_back"><i className="fas fa-chevron-left"></i>Go back</Link>
      <div className="quiz_container">
        <h1 className="title">
          Let's see how much do know about triangles ?
        </h1>
            ...
            return (
              <div key={question.id} className="questions">
                ...
                <select id={question.id} value={arr[turn[question.id]]} onChange={handleChange}>
                  ...
                  <option value={question.options[3]}>{question.options[3]}</option>
                </select>
              </div>
            )
          })}
        </div>
        <button className="quiz_button" onClick={handleSubmit}>Submit</button>
